...
try {
    ...
    $patterns = @(
        ...
        @{ pattern = '\bgit\b[^;&|\n]*\s--no-verify\b';                 reason = 'git --no-verify (bypasses pre-commit/pre-push hooks)' },
        @{ pattern = '\bgit\s+commit\b[^;&|\n]*\s-n\b';                 reason = 'git commit -n (--no-verify shorthand, bypasses hooks)' },
        @{ pattern = '\bgit\b[^;&|\n]*\s-c\s+core\.hooksPath\s*=';      reason = 'git -c core.hooksPath= (disables git hooks)' },
